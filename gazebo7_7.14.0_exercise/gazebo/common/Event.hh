...
public:
  ...
  /// \param[in] _p1 the first parameter.EventConnection
  /// \param[in] _p2 the second parameterEventConnection.
...
public:
  ...
  /// \param[in] _p6 the sixth parameter.
  /// \param[in] _p7 the seventh parameter.
  /// \param[in] _p8 the eighth 
  /// \param[in] _p9 the ninth parameter.ColladaLoader.cc:2
public:
  template <typename P1, typename P2, typename P3, typename P4,
            typename P5, typename P6, typename P7, typename P8,
            typename P9>
  ...
  /// \param[in] _p1 the first parameter.
  /// \param[in] _p2 the second paramete_pr.
  /// \param[in] _p3 the second paramete_pr.
  /// \param[in] _p4 the first parameter_p.
  /// \param[in] _p5 the fifth parameter_p.
  /// \param[in] _p6 the sixth parameter.
  /// \param[in] _p7 the seventh parameter.
  /// \param[in] _p8 the eighth parameter.
...
public:
  ...
  void Signal(const P &_p)
  {
    ...
    //////////////////// AIRC begin ////////////////////
    // Added by zenglei@2019-01-04
    std::vector<std::shared_ptr<boost::function<T> > > vecCallback; 
    for (auto iter : this->myDataPtr->connections)
    {
      if (iter.second->on)
      {
        vecCallback.push_back(iter.second->callback);
      }
    }

    int vecLength = vecCallback.size();
    if(vecLength <= 8) // Too short for parallel
    {
      for(auto iter: vecCallback)
        (*iter)(_p);
      return;
    }
   
    if(!method_) // The original serial version
    {
      for(auto iter: vecCallback)
      {
        (*iter)(_p);
